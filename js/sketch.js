...
function randomCosmeticConfig(){
  return {
    ...
    // chaos scatters each symmetry arm with its own independent random
    // offset, which breaks the precise radial symmetry that makes it read
    // as a mandala in the first place — always off for ambient idle art
    chaos: 0,
...
function drawArm(pdx, pdy, dx, dy, sw, strokeColour){
  const jpdx = jitter(pdx), jpdy = jitter(pdy), jdx = jitter(dx), jdy = jitter(dy);
  ...
  maybeSpawnParticles(jdx, jdy, strokeColour);
  ...
  if (strokeStyleMode === 'line'){
    ...
    artLayer.line(jpdx, jpdy, jdx, jdy);
    ...
    artLayer.ellipse(jdx, jdy, sw, sw);

  } else if (strokeStyleMode === 'sparkle'){
    artLayer.noStroke();
    artLayer.fill(strokeColour);
    drawStar(jdx, jdy, sw * 0.4, sw * 1.3, 4);
  }
}
